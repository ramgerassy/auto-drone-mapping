...
| 4b | `feat/perception-teammate-filter` | `perception`: filter teammate returns out of a scan so drones are never mapped as obstacles (emit a MISS with shortened range) | simulation, perception | ✅ implemented — PR pending — [plan](sprint-2/feature-4b-perception-teammate-filter.md) |
| 4c | `feat/planner-clearance` | **Drone body vs config, both halves.** `planning`: obstacle inflation so the 0.30 m body is not planned as a point (clearance radius in `AStarPlanner`, always-traversable start cell, known-occupied cells only). `simulation`: half-extent as a module constant. `coordination`: `min_separation` floor at the body diagonal (0.424 m), start-position spacing validation, `min_separation_cells >= 1` assertion | 2, 4 | ✅ implemented — PR pending — [plan](sprint-2/feature-4c-planner-clearance.md) |
| 5 | `feat/large-indoor` | `scenarios/large_indoor/` MJCF (50×50, corridors, doorways) + config, with the open-top/lighting/handle-camera treatment. **Doorways/corridors must exceed `2r + 1` cells PLUS one cell of discretization slop** — stated in *cells*, not metres: a gap sized to the theoretical minimum maps one cell narrower than it is and refuses the drone (measured at `resolution` 0.25; see progress.md) | 4c (design constraint) | ☐ |
